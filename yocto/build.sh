...
do_clean_build ()
{
    echo "do_clean_build start"

    bitbake -c cleanall world

    echo "do_cldo_clean_buildean done"
}
...
do_prep_host ()
{
    ...
    # backup original
    cp ${BUILD_DIR}/conf/bblayers.conf ${BUILD_DIR}/conf/bblayers_orig.conf
    cp ${BUILD_DIR}/conf/local.conf ${BUILD_DIR}/conf/local_orig.conf
    ...
    cp ${YOCTO_DIR}/conf/${TARGET_ARCH}/local_conf_${GITBRANCH_COMMAND}.inc ${BUILD_DIR}/conf/

    # sed local.conf
    sed -i "s|<git_branch>|${GITBRANCH_COMMAND}|g" "${BUILD_DIR}/conf/local.conf"
    ...
    sed -i "s|<BUILD_DIRECTORY>|${BUILD_DIRECTORY}|g" "${BUILD_DIR}/conf/local.conf"
...
source ${PWD}/.config
source ${PWD}/../requirements/.config.requirements.sh
